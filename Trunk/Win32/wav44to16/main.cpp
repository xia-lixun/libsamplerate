...
int main(int argc, char *argv[])
{
	...
	if (w44.GetSampleRate() == SOURCE_SAMPLE_RATE)
	{
	...
	else if (w44.GetSampleRate() == TARGET_SAMPLE_RATE)
	{
		const size_t channels = w44.GetNumChannel();
		const size_t w44samples = w44.GetFrameLength();
		...
		for (size_t chan = 0; chan < channels; ++chan)
		{
			...
			Cwav w16;
			float *w16x = w16.SetFrameMatrix((int)count, 1, TARGET_SAMPLE_RATE);
			memcpy(w16x, &w44track[0], sizeof(float) * count);
		...
		std::cout << "error: sample rate mismatch!\n";
	}
